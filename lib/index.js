...
class TokenUsageStore {
  ...
  /**
   * 解析当天出现过的模型 id → 显示名（带 TTL 缓存）。
   * 优先用 llm.listModels(provider)（pi-ai 直接返回配置里的 name），
   * 失败则逐个 resolveModelInfo；解析失败/无显示名时该模型保持原样（null）。
   */
  async resolveNames() {
    if (!this.llm) return new Map();
    const now = Date.now();
    if (this.nameCache && now - this.nameCache.builtAt < NAME_CACHE_TTL_MS) return this.nameCache.map;
    const providers = new Set();
    for (const b of this.models.values()) if (b.provider) providers.add(b.provider);
    const map = new Map();
    for (const provider of providers) {
      try {
        const infos = await this.llm.listModels(provider);
        for (const info of infos || []) {
          if (info && info.id && typeof info.name === "string" && info.name.length > 0) map.set(info.id, info.name);
        }
      } catch {
        // 适配器不支持批量列出：逐个解析
        for (const [model, b] of this.models) {
          if (b.provider !== provider || map.has(model)) continue;
          try {
            const info = await this.llm.resolveModelInfo(provider, model, AbortSignal.timeout(3000));
            if (info && typeof info.name === "string" && info.name.length > 0) map.set(model, info.name);
          } catch {
            /* 解析失败：保持原样 */
          }
        }
    ...
    this.nameCache = { builtAt: now, map };
    return map;
  }
  ...
  async todayView() {
    ...
    if (this.llm) {
      ...
      for (const row of models) {
        const name = names.get(row.model);
  ...
  ctx.inject(["connection"], (connectionCtx) => {
    connectionCtx.connection.rpc.handle(
      ...
      async (endpoint) => {
        ...
        return { ok: true, value: await store.todayView() };
      },
