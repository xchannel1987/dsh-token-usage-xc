...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".dshu-pillSuffix{font-size:inherit;line-height:inherit}",
			...
			// 移动端（<=1023px，与 DSH vendor SIDEBAR_AUTO_COLLAPSE / dsh-mobile-xc 断点一致）：
			// 1) 会话顶栏 actions 组内厂商项（agent 预设标签、上下文占用表计）不收缩，整行会溢出视口，
			//    排在末尾的徽标被 overflow:hidden 祖先裁出屏幕（"不见了"）。让徽标排到 actions 组最前。
			// 2) 顶栏第一行整体排布：汉堡 | PTC 模式 | 剩余表计 | 用量徽标 | 后台任务(jobs)（徽标倒数第二、jobs 最右）。
			//    order 控制：PTC/剩余默认 0 → 徽标 20 → jobs 30；徽标移动端去 tok 后缀 + 收紧 padding；≤380px 自动换行保序。
			"@media (max-width:1023px){.dshu-pillSuffix{display:none}.dshu-pill{padding:2px 3px}[data-slot=\"conversation.session.header.actions\"] .dshu-pillRoot{order:20}[data-slot=\"conversation.session.header.actions\"]>[class*=\"_root\"]{order:30;flex-basis:auto;margin-left:0}[class*=\"_headerActions\"]{gap:5px;flex-wrap:nowrap;flex:none}}@media (max-width:380px){[class*=\"_headerActions\"]{flex-wrap:wrap;flex:0 0 100%}}",
		].join("");
		...
		function createController(rpc, scope) {
			...
			async function refreshOne(targetStore, endpoint) {
				...
				try {
					const out = await rpc.call("/dsh-token-usage-xc", endpoint, {});
		...
		function UsageBadge({ t, useTokenUsage, useTokenUsageConfig, useTokenUsageDays, refresh }) {
			...
			return h("div", { className: cx.pillRoot, ref: rootRef }, [
				h("button", { type: "button", className: cx.pill, "aria-expanded": open, "aria-label": t("pillAria"), onClick: () => setOpen((v) => !v) }, [
					total !== null && usage.phase !== "loading" ? h("span", { className: cx.pillText }, [compactTokens(total), h("span", { className: cx.pillSuffix }, " tok")]) : h("span", { className: cx.pillText }, label)
				]),
		...
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "dsh-token-usage-xc: dictionaries");
